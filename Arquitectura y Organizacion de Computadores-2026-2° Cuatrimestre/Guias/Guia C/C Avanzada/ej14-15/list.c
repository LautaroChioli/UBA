...
void* nodoGet(list_t* l, uint8_t i){
    node_t* n = l->first;
    for(uint8_t j = 0; j < i; j++)
    n = n->next;
    return n;
...
void cambiarNodo(list_t* l, uint8_t n1, uint8_t n2){
    if (l->first == NULL){return;}
    if (l->size == 1){return;}

    node_t* nodo1 = nodoGet(l, n1);
    node_t* nodo2 = nodoGet(l, n2);

    void* temp_data = nodo1->data;
    nodo1->data = nodo2->data;
    nodo2->data = temp_data;
